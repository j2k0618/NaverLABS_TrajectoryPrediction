...
class NusTrajectoryExtractor:
    ...
    def get_cmu_annotation(self, instance_tokens, sample_token):
        ...
        scene_annotation = scene_data['annotation']
        ...
        return {'episode': episode, 'episode_img': episode_img, 'img_show': map_image_show, 'scene_txt': scene_annotation}
        ...
        for split in tqdm(split_types, desc='split dataset'):
            ...
            for sample_tk, instance_tks in tqdm(tokens_dict.items(), desc=split, total=len(tokens_dict)):
                ...
                if not os.path.isdir(sample_dir):
                    ...
                    with open('{}/scene_txt.bin'.format(sample_dir), 'wb') as f:
                        pickle.dump(scene_data['scene_txt'], f, pickle.HIGHEST_PROTOCOL)
...
class NusCustomDataset(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        past, past_len, future, future_len, agent_mask, vel, pos, _, _ = self.episodes[idx]
...
def nuscenes_collate(batch, test_set=False, val=False):
    ...
    if not val:
        theta_list = [90, 180, -90, -45, 45, 135, -135, 12, -12, 178, -178, 89, 1]
