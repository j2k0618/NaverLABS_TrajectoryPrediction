...
class CrossModalAttention(nn.Module):
    ...
    def forward(self, map_features, traj_encoding, episode_idx):
        ...
        # map_fimage = map_features[episode_idx].mul_(alpha).reshape(1,32,32,-1)[0].mean(dim=2)
...
class CrossModalGoalDecoder(nn.Module):
    ...
    def forward(self, z, x_flat, static, dx, x_prev, global_scene_encoding, episode_idx):
        ...
        # Detect dynamic batch size
        batch_size = static.size(0)
        num_agent = int(batch_size/10)
        agent_mask = [10*i for i in range(num_agent)]
        # print(num_agent)
        # print(batch_size)
        
        # Unroll a step
        dynamic_encoding = self.mlp_goal(x_flat.unsqueeze(0))[-1]
        goal_encoding = self.mlp_goal(x_flat[agent_mask].unsqueeze(0))[-1]


        att_scene, alpha = self.crossmodal_attention(global_scene_encoding, dynamic_encoding, episode_idx)
        att_goal_scene, _ = self.crossmodal_attention(global_scene_encoding, goal_encoding, episode_idx[agent_mask])

        # Concat the dynamic and static encodings
        dynamic_static = torch.cat((dynamic_encoding, static, att_scene), dim=-1) # B X 232
        goal_static = torch.cat((goal_encoding, static[agent_mask], att_goal_scene), dim=-1) # B X 232
...
class CrossModalPathDecoder(nn.Module):
    ...
    def __init__(self, image_dim=32, hidden_size=150, context_dim=50, decoding_steps=5, velocity_const=0.5, att=True, goal_enc_dim=232):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Linear(100, 50),
            nn.ReLU(),
            nn.Linear(50, 2 * self.decoding_steps) # DON'T USE ACTIVATION AT THE TOP MOST LAYER
        )
    ...
    def forward(self, z, x_flat, static, dx, x_prev, global_scene_encoding, episode_idx, goal, goal_encoding):
        ...
        # print(dynamic_encoding.size(), static.size(), att_scene.size(), goal_encoding.size())
