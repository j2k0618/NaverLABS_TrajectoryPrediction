...
class ModelTrainer:
    ...
    def __init__(self, goal_model, path_model, train_loader, valid_loader, optimizer, exp_path, args, device, ploss_criterion_mse=None, ploss_criterion_map=None, path_ploss_criterion_mse=None, path_ploss_criterion_map=None):
        ...
        self.ploss_criterion_map = ploss_criterion_map
        self.ploss_criterion_mse = ploss_criterion_mse
        self.path_ploss_criterion_map = path_ploss_criterion_map
        self.path_ploss_criterion_mse = path_ploss_criterion_mse
        self.path_weight = args.path_weight

        self.goal_opt = optimizer[0]
        self.path_opt = optimizer[1]
        # self.scheduler = ReduceLROnPlateau(self.optimizer, factor=(1 / 2), verbose=True, patience=3)
        self.goal_scheduler = ReduceLROnPlateau(self.goal_opt, factor=(1 / 2), verbose=True, patience=3)
        self.path_scheduler = ReduceLROnPlateau(self.path_opt, factor=(1 / 2), verbose=True, patience=3)
    ...
    def train(self, num_epochs):
        ...
        for epoch in tqdm(range(self.start_epoch, self.start_epoch + num_epochs)):
            ...
            self.goal_scheduler.step(scheduler_metric[0])
            self.path_scheduler.step(scheduler_metric[1])
            ...
            logging_msg2 = (
                ...
                f'| Goal Learning Rate: {self.get_goal_lr():g}\n'
                f'| Path Learning Rate: {self.get_path_lr():g}\n'
            )
        ...
        for b, batch in enumerate(self.train_loader):
            ...
            if self.beta != 0.0:
                if self.ploss_type == 'mseloss':
                    ...
                    ploss = self.ploss_criterion_mse(gen_goals, future_agents_traj[:,-1,:].unsqueeze(1))
                else:
                    ...
                    ploss = 0.1 * self.ploss_criterion_map(episode_idx, gen_goals, log_prior, -15.0) + self.path_ploss_criterion_mse(gen_goals, future_agents_traj[:,-1,:].unsqueeze(1))
            ...
            if self.beta != 0.0:
                if self.path_ploss_type == 'mseloss':
                    # ploss = self.ploss_criterion(gen_trajs, past_agents_traj)
                    ploss = 5 * self.path_ploss_criterion_mse(gen_trajs, future_agents_traj[:,:-1,:])
                else:
                    ploss = 5 * self.path_ploss_criterion_map(episode_idx, gen_trajs, log_prior, -15.0) + self.path_ploss_criterion_mse(gen_trajs, future_agents_traj[:,:-1,:])
            ...
            for b, batch in enumerate(self.valid_loader):
                ...
                if self.beta != 0.0:
                    if self.ploss_type == 'mseloss':
                        ...
                        ploss = self.ploss_criterion_map(episode_idx, gen_goals, log_prior, -15.0)
    ...
    def get_goal_lr(self):
        """Returns Learning Rate of the Optimizer."""
        for param_group in self.goal_opt.param_groups:
            return param_group['lr']

    def get_path_lr(self):
        """Returns Learning Rate of the Optimizer."""
        for param_group in self.path_opt.param_groups:
            return param_group['lr']
    ...
    def save_checkpoint(self, epoch, ade, fde, qloss=0., ploss=0.):
        ...
        state_dict = {
            ...
            'goal_optimizer': self.goal_opt.state_dict(),
            'path_optimizer': self.path_opt.state_dict(),
            'goal_learning_rate': self.get_goal_lr(),
            'path_learning_rate': self.get_path_lr(),
    ...
    def load_checkpoint(self, ckpt):
        ...
        self.goal_opt.load_state_dict(checkpoint['goal_optimizer'])
...
class ModelTest:
        ...
        for test_time in range(self.test_times):
            ...
            with torch.no_grad():
                ...
                for b, batch in enumerate(self.data_loader):
                    ...
                    decode_start_vel, decode_start_pos, scene_id, _ = batch
                    ...
                    for candi in range(gen_goals.size(1)):
                        ...
                        gen_trajs = self.path_model(motion_encoding_, src_lens, agent_tgt_three_mask,
                                                                 episode_idx, decode_start_vel, decode_start_pos,
                                                                 num_src_trajs, scene_encoding_, gen_goals[:,candi].squeeze(1), goal_encoding.detach())
                        gen_trajs = torch.cat((gen_trajs, gen_goals[:,candi].unsqueeze(1)), dim=2)
                        gen_trajs_list.append(gen_trajs)
    ...
    def run(self):
        ...
        for test_time in range(self.test_times):
            ...
            with torch.no_grad():
                ...
                for b, batch in enumerate(self.data_loader):
                    ...
                    rs_error3 = rs_error6[..., :int((self.decoding_steps + self.path_decoding_steps) / 2)]
                    ...
                    for agent_idx, paths in enumerate(gen_trajs):
                        ...
                        # TODO: index 5 -> 11
                        f_points = paths[:5, 11, :].squeeze() - decode_start_pos[agent_idx]
        ...
        print("minFSD6_5: {:.5f}".format(minFSD3.mean()))
        print("stdFD6_5: {:.5f}".format(stdFD3.mean()))
